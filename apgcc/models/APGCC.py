...
class Model_builder(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        self.cfg = cfg
        self.num_classes = self.cfg.MODEL.DECODER_kwargs[
            "num_classes"
        ]  # default:2 (person/background)
    ...
    def _build_encoder(
        ...
        elif self.cfg.MODEL.ENCODER in [
            "resnet18",
            "resnet34",
            "resnet50",
            "resnet101",
...
class SetCriterion_Crowd(nn.Module):
    ...
    def loss_auxiliary(self, outputs, targets, show):
        ...
        for n_pos in range(self.aux_number[0]):
            ...
            # cls loss
            pred_logits = src_outputs[
                "pred_logits"
            ]  # size=[1, # of gt anchors, 2] [p0, p1]
            target_classes = torch.ones(
                pred_logits.shape[:2], dtype=torch.int64, device=pred_logits.device
            )  # [1, # of gt anchors], all sample is the head class
            ...
            target_points = target_points.repeat(
                1, int(pred_points.shape[0] / target_points.shape[0])
            )
        ...
        for n_neg in range(self.aux_number[1]):
            ...
            target_points = torch.zeros(
                pred_points.shape, dtype=torch.float, device=pred_logits.device
            )
        ...
        if show:
            if self.aux_number[0] > 0:
                print(
                    "Auxiliary Training: [Pos] loss_cls:",
                    loss_ce_pos,
                    " loss_loc:",
                    loss_loc_pos,
                    " loss:",
            ...
            if self.aux_number[1] > 0:
                print(
                    "Auxiliary Training: [Neg] loss_cls:",
                    loss_ce_neg,
                    " loss_loc:",
                    loss_loc_neg,
                    " loss:",
        ...
        loss_aux = (
            self.aux_kwargs["pos_coef"] * loss_aux_pos
            + self.aux_kwargs["neg_coef"] * loss_aux_neg
        )
    ...
    def forward(self, outputs, targets, show=False):
        ...
        indices1 = self.matcher(
            output1, targets
        )  # return (idx_of_pred, idx_of_gt). # pairs of indices # indices[batch] = (point_coords, gt_idx)
