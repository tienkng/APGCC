...
class RegressionModel(nn.Module):
    def __init__(self, num_features_in, num_anchor_points=4, feature_size=256):
        ...
        self.conv2 = nn.Sequential(
            nn.Conv2d(feature_size, feature_size, kernel_size=3, padding=1), nn.ReLU()
        )
...
class ClassificationModel(nn.Module):
    def __init__(
        self, num_features_in, num_anchor_points=4, num_classes=2, feature_size=256
    ):
    ...
    def forward(self, x):
        ...
        return out2.contiguous().view(
            x.shape[0], -1, self.num_classes
        )  # check it to be [bs, num_of_queries, classes]
...
class FPN(nn.Module):
    def __init__(
        ...
        if 4 in self.feat_layers:
            ...
            self.P5_1 = nn.Conv2d(
                C5_size, inner_planes, kernel_size=1, stride=1, padding=0
            )
            self.P5_upsampled = nn.Upsample(scale_factor=2, mode="nearest")
            self.P5_2 = nn.Conv2d(
                inner_planes, inner_planes, kernel_size=3, stride=1, padding=1
            )

        if 3 in self.feat_layers:
            # add P5 elementwise to C4
            self.P4_1 = nn.Conv2d(
                C4_size, inner_planes, kernel_size=1, stride=1, padding=0
            )
        ...
        if 2 in self.feat_layers:
            ...
            self.P3_1 = nn.Conv2d(
                C3_size, inner_planes, kernel_size=1, stride=1, padding=0
            )
        ...
        if 1 in self.feat_layers:
            ...
            self.P2_1 = nn.Conv2d(
                C2_size, inner_planes, kernel_size=1, stride=1, padding=0
            )
...
class ASPP(nn.Module):
    ...
    def forward(self, x):
        ...
        _, _, h, w = x.size()
        feat1 = F.upsample(
            self.conv1(x).to(device), size=(h, w), mode="bilinear", align_corners=True
        )
        feat2 = self.conv2(x)
        feat3 = self.conv3(x)
        feat4 = self.conv4(x)
...
class ifi_simfpn(nn.Module):
    def __init__(
        ...
        norm_layer = (
            nn.SyncBatchNorm if sync_bn else nn.BatchNorm1d
        )  # nn.BatchNorm1d / nn.InstanceNorm2d
        ...
        if learn_pe:
            ...
            for level in range(self.feat_num):
                self._update_property(
                    "pos" + str(level + 1), PositionEmbeddingLearned(self.pos_dim // 2)
                )
        elif ultra_pe:
            print("ultra_pe")
            for level in range(self.feat_num):
        ...
        for ct, hidden_feature in enumerate(head_layers):
            ...
            confidence_head_list.append(
                [
                    nn.Conv1d(src_dim, hidden_feature, 1),
                    norm_layer(hidden_feature),
                    nn.ReLU(),
                ]
            )
            offset_head_list.append(
                [
                    nn.Conv1d(src_dim, hidden_feature, 1),
                    norm_layer(hidden_feature),
                    nn.ReLU(),
        ...
        confidence_head_list.append(
            [
                nn.Conv1d(
                    head_layers[-1], self.num_anchor_points * self.num_classes, 1
                ),
                nn.ReLU(),
            ]
        )
        offset_head_list.append(
            [nn.Conv1d(head_layers[-1], self.num_anchor_points * 2, 1)]
        )

        # build heads
        confidence_head_list = [
            item for sublist in confidence_head_list for item in sublist
        ]
    ...
    def forward(self, x, size, level=0, after_cat=False):
        ...
        if not after_cat:
            if not self.local:
                ...
                rel_coord_list, q_feat_list, area_list = ifi_feat(
                ...
                for rel_coord, q_feat, area in zip(
                    ...
                    if self.ultra_pe:
                        ...
                        rel_coord = eval("self.pos" + str(level))(
                            rel_coord, [1, 1, h, w]
                        )
            ...
            confidence = confidence.permute(
                0, 2, 3, 1
            )  # # b, h, w, line*cow*num_classes
            confidence = confidence.contiguous().view(
                x.shape[0], -1, self.num_classes
            )  # b, num_queries, self.num_classes
...
def ifi_feat(res, size, stride=1, local=False):
    ...
    h, w = size
    device = res.device  # Lấy thiết bị từ res
    coords = (make_coord((h, w), device=device).flip(-1) + 1) / 2
    coords = coords.unsqueeze(0).expand(bs, *coords.shape)
    coords = (coords * 2 - 1).flip(-1)

    feat_coords = (
        make_coord((hh, ww), flatten=False, device=device)
        .permute(2, 0, 1)
        .unsqueeze(0)
